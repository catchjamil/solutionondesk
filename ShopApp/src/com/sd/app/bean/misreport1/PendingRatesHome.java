 ...
 * @see com.sd.app.bean.misreport1.PendingRates
...
public class PendingRatesHome {
	...
	public PendingRates findById(com.sd.app.bean.misreport1.PendingRatesId id) {
		log.debug("getting PendingRates instance with id: " + id);
		try {
			PendingRates instance = (PendingRates) sessionFactory.getCurrentSession().get("com.sd.app.bean.misreport1.PendingRates", id);
			if (instance == null) {
	...
	public List<PendingRates> findByExample(PendingRates instance) {
		...
		try {
			List<PendingRates> results = (List<PendingRates>) sessionFactory.getCurrentSession()
					.createCriteria("com.sd.app.bean.misreport1.PendingRates").add(create(instance)).list();
