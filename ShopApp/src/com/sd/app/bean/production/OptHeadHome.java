 ...
 * @see com.sd.app.bean.production.OptHead
...
public class OptHeadHome {
	...
	public OptHead findById(com.sd.app.bean.production.OptHeadId id) {
		log.debug("getting OptHead instance with id: " + id);
		try {
			OptHead instance = (OptHead) sessionFactory.getCurrentSession().get("com.sd.app.bean.production.OptHead", id);
			if (instance == null) {
	...
	public List<OptHead> findByExample(OptHead instance) {
		...
		try {
			List<OptHead> results = (List<OptHead>) sessionFactory.getCurrentSession().createCriteria("com.sd.app.bean.production.OptHead")
