 ...
 * @see com.sd.app.bean.wipfile.TmpFile
...
public class TmpFileHome {
	...
	public TmpFile findById(com.sd.app.bean.wipfile.TmpFileId id) {
		log.debug("getting TmpFile instance with id: " + id);
		try {
			TmpFile instance = (TmpFile) sessionFactory.getCurrentSession().get("com.sd.app.bean.wipfile.TmpFile", id);
			if (instance == null) {
	...
	public List<TmpFile> findByExample(TmpFile instance) {
		...
		try {
			List<TmpFile> results = (List<TmpFile>) sessionFactory.getCurrentSession().createCriteria("com.sd.app.bean.wipfile.TmpFile")
