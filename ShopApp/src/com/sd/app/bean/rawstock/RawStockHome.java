 ...
 * @see com.sd.app.bean.rawstock.RawStock
...
public class RawStockHome {
	...
	public RawStock findById(com.sd.app.bean.rawstock.RawStockId id) {
		log.debug("getting RawStock instance with id: " + id);
		try {
			RawStock instance = (RawStock) sessionFactory.getCurrentSession().get("com.sd.app.bean.rawstock.RawStock", id);
			if (instance == null) {
	...
	public List<RawStock> findByExample(RawStock instance) {
		...
		try {
			List<RawStock> results = (List<RawStock>) sessionFactory.getCurrentSession().createCriteria("com.sd.app.bean.rawstock.RawStock")
