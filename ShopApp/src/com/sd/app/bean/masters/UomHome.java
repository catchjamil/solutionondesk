 ...
 * @see com.sd.app.bean.masters.Uom
...
public class UomHome {
	...
	public Uom findById(com.sd.app.bean.masters.UomId id) {
		log.debug("getting Uom instance with id: " + id);
		try {
			Uom instance = (Uom) sessionFactory.getCurrentSession().get("com.sd.app.bean.masters.Uom", id);
			if (instance == null) {
	...
	public List<Uom> findByExample(Uom instance) {
		...
		try {
			List<Uom> results = (List<Uom>) sessionFactory.getCurrentSession().createCriteria("com.sd.app.bean.masters.Uom").add(create(instance))
					.list();
