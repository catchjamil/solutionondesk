 ...
 * @see com.sd.app.bean.misreport.Summary
...
public class SummaryHome {
	...
	public Summary findById(com.sd.app.bean.misreport.SummaryId id) {
		log.debug("getting Summary instance with id: " + id);
		try {
			Summary instance = (Summary) sessionFactory.getCurrentSession().get("com.sd.app.bean.misreport.Summary", id);
			if (instance == null) {
	...
	public List<Summary> findByExample(Summary instance) {
		...
		try {
			List<Summary> results = (List<Summary>) sessionFactory.getCurrentSession().createCriteria("com.sd.app.bean.misreport.Summary")
