 ...
 * @see com.sd.app.bean.samples.Stock
...
public class StockHome {
	...
	public Stock findById(com.sd.app.bean.samples.StockId id) {
		log.debug("getting Stock instance with id: " + id);
		try {
			Stock instance = (Stock) sessionFactory.getCurrentSession().get("com.sd.app.bean.samples.Stock", id);
			if (instance == null) {
	...
	public List<Stock> findByExample(Stock instance) {
		...
		try {
			List<Stock> results = (List<Stock>) sessionFactory.getCurrentSession().createCriteria("com.sd.app.bean.samples.Stock")
