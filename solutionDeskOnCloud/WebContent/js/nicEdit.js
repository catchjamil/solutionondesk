...
var nicEditor = bkClass.extend({
	...
	getIcon : function(iconName,options) {
		...
		var backgroundImage1 = "url('"+((icon) ? this.options.iconsPath : file)+"')";
		var  backgroundPosition1 = ((icon) ? ((icon-1)*-18) : 0)+'px 0px';
...
var nicUploadButton = nicEditorAdvancedButton.extend({	
	 nicURI : 'http://solutiondesk.cloudfoundry.com/uploadImage.do',
  ...
  onUploaded : function(options) {
    this.removePane();
    var src = options.result[0];
    var imgWidth = options.result[1];
    if(!this.im) {
      this.ne.selectedInstance.restoreRng();
      var tmp = 'javascript:nicImTemp();';
    ...
    if(this.im) {
      this.im.setAttributes({
        src : src,
        // width : (w && imgWidth) ? Math.min(w, imgWidth) : ''
      });
    }
  }
});
